...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetUntilmod3
(
JNIEnv *env,
jclass cls,
...
//		if (soln[i]) printf("yes[%d] := %f;\n", i+1, yes[i]);
	...
	int* is_usefull = new int[n];

	FILE* fl2 = fopen(str, "r+");
	for(i = 0; i < n; i++)
		fscanf(fl2,"%d", &is_usefull[i]);
	fclose(fl2);
	if(min)
		for(i = 0; i < n; i++)
