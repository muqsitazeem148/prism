...
JNIEXPORT jlong __jlongpointer JNICALL Java_sparse_PrismSparse_PS_1NondetUntilmod1
(
JNIEnv *env,
jclass cls,
...
//		if (soln[i]) printf("yes[%d] := %f;\n", i+1, yes[i]);
	...
	printf("\nUSEFULESTATE = %d and uf ratio = %f\n", right, right / (float)ufst);

	printf("Useful and useless states:\n");
	for(i = 0; i < n; i++)
